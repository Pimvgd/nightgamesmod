...
public class ForceBreastStrategy extends Skill {
    public ForceBreastStrategy(Character self) {
        super("Force Breast Strategy", self, 5);
    }
    ...
    public int accuracy(Combat c, Character target) {
        return target.is(Stsflag.blinded) ? -100 : 990;
    }
    ...
    public boolean resolve(Combat c, Character target) {
        ...
        if (result == Result.normal) {
            c.getCombatantData(target).setStrategy(c, target, new BreastStrategy() {
                public int initialDuration(Combat c, Character ch) {
                    return 9999;
                }
            });
    ...
    public Skill copy(Character user) {
        return new ForceBreastStrategy(user);
    }
